...
def talker():
    ...
    while not rospy.is_shutdown():
        ...
        Lattitude.append(float(
            data2[6])+(0.5*cos(pi/180*float(data2[3]))-0.1*sin(pi/180*float(data2[3])))/6356752*180/pi)
        Longitude.append(float(data2[7])+(0.5*sin(pi/180*float(data2[3]))+0.1*cos(
            pi/180*float(data2[3])))/6365986/cos(pi/180*Lattitude[-1])*180/pi)
        ...
        route_dic = {'GPSTime': GPSTime, 'Heading': Heading, 'Pitch': Pitch, 'Roll': Roll, 'Lattitude': Lattitude, 'Longitude': Longitude,
                     'Altitude': Altitude, 'Ve': Ve, 'Vn': Vn, 'Vu': Vu, 'Baseline': Baseline, 'NSV1': NSV1, 'NSV2': NSV2, 'Status': Status, 'La2': La2, 'Lo2': Lo2}
